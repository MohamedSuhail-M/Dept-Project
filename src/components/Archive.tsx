    ...
    <main className="bg-forest pt-32 text-cream">
      <section className="border-b border-cream/12 px-5 pb-16 lg:px-10 lg:pb-24">
        <div className="mx-auto max-w-[1440px]">
          <Reveal>
            ...
            <h1 className="display mt-6 max-w-4xl">NAC <em className="text-lime">Digital</em><br />Archive.</h1>
            <p className="mt-8 max-w-xl text-lg leading-8 text-cream/60">Academic documentation, evidence and quality assurance records — organised by file, subsection and academic year.</p>
            <div className="mt-14 grid grid-cols-3 gap-px bg-cream/12 py-px md:max-w-2xl">
              {[['25', 'FILES'], ['100+', 'DOCUMENT TYPES'], ['MULTI', 'YEAR ARCHIVE']].map(([value, label]) => (
      ...
      <section className="mx-auto max-w-[1440px] px-5 py-16 lg:px-10 lg:py-24">
        ...
        <div className="grid gap-px bg-cream/10 sm:grid-cols-2 lg:grid-cols-3">
          {nacStructure.map((file) => <FileCard key={file.fileNumber} file={file} />)}
        </div>
      </section>
    </main>
  );
  ...
  return (
    <Reveal>
      <a href={`/nac/${file.fileNumber}`} className="group block bg-forest p-6 transition-colors hover:bg-deep-emerald md:p-8">
        ...
        <span className="mt-8 inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.18em] text-lime">EXPLORE <ArrowUpRight size={14} className="transition group-hover:translate-x-1 group-hover:-translate-y-1" /></span>
      </a>
    </Reveal>
  );
}
...
export function CategoryPage({ fileNumber }: { fileNumber: number }) {
  ...
  useEffect(() => { if (file) fetchDocuments({ fileNumber }).then(setDocs).catch(() => setDocs([])); }, [fileNumber, file]);
  if (!file) return <NotFound />;
  return (
    <main className="bg-forest px-5 pb-24 pt-32 text-cream lg:px-10">
      <div className="mx-auto max-w-[1200px]">
        <Reveal>
          <a href="/nac" className="inline-flex items-center gap-2 text-[11px] font-bold tracking-[0.16em] text-lime/70 hover:text-lime transition"><ChevronLeft size={15} /> NAC FILES</a>
        ...
        <div className="mt-16 border-t border-cream/12">
          {file.headings?.map((h) => (
            <div key={h.label} className="border-b border-cream/12 bg-deep-emerald px-5 py-5 text-sm font-bold tracking-[0.12em] text-lime">{h.label}</div>
          ))}
          {file.sections.map((section) => {
            ...
            return (
              <div key={section.number} className="group grid gap-5 border-b border-cream/12 py-7 md:grid-cols-[90px_1fr_auto] md:items-center">
                <span className="font-mono text-sm text-lime/70">{section.number === 0 ? '—' : String(section.number).padStart(2, '0')}</span>
                <div>
                  ...
                  <p className="mt-2 text-[10px] font-bold uppercase tracking-[0.15em] text-cream/40">{sectionDocs.length ? `${sectionDocs.length} DOCUMENT${sectionDocs.length === 1 ? '' : 'S'}` : 'NO DOCUMENTS YET'}</p>
                </div>
                <a href={`/nac/${file.fileNumber}/${section.number}`} className="inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.16em] text-lime hover:text-cream transition">OPEN <ArrowUpRight size={14} /></a>
              </div>
            );
          })}
        </div>
...
export function SubsectionPage({ fileNumber, subsectionNumber }: { fileNumber: number; subsectionNumber: number }) {
  ...
  useEffect(() => { if (file && subsection) fetchDocuments({ fileNumber, subsectionNumber, academicYear: year || undefined }).then(setDocs).catch(() => setDocs([])); }, [fileNumber, subsectionNumber, year, file, subsection]);
  const filtered = useMemo(() => docs.filter((doc) => (`${doc.title} ${doc.filename}`).toLowerCase().includes(search.toLowerCase())), [docs, search]);
  if (!file || !subsection) return <NotFound />;
  return (
    <main className="bg-forest px-5 pb-24 pt-32 text-cream lg:px-10">
      <div className="mx-auto max-w-[1100px]">
        <Reveal>
          <a href={`/nac/${file.fileNumber}`} className="inline-flex items-center gap-2 text-[11px] font-bold tracking-[0.16em] text-lime/70 hover:text-lime transition"><ChevronLeft size={15} /> {file.title}</a>
        ...
        <div className="mt-10 grid gap-3 border-y border-cream/12 py-4 md:grid-cols-[1fr_220px_auto]">
          ...
          <select value={year} onChange={(e) => setYear(e.target.value)} className="field">
            ...
            {academicYears.map((item) => <option key={item}>{item}</option>)}
          </select>
          <span className="inline-flex items-center justify-center gap-2 px-3 text-[10px] font-bold tracking-[0.16em] text-cream/45"><SlidersHorizontal size={14} /> {filtered.length} RESULTS</span>
        </div>
        <div className="mt-10 space-y-3">
          {filtered.length ? filtered.map((doc) => <DocumentRow key={doc.id} document={doc} />) : (
...
function DocumentRow({ document }: { document: NACDocument }) {
  ...
  const open = async () => {
    try {
      const next = await getDocumentUrl(document);
      setUrl(next);
  ...
  return (
    <article className="flex flex-col gap-5 border border-cream/12 bg-deep-emerald p-5 md:flex-row md:items-center md:justify-between">
      <div className="flex items-start gap-4">
        <div className="grid h-11 w-11 shrink-0 place-items-center bg-lime text-forest"><FileText size={19} /></div>
        <div>
          <h2 className="font-bold text-cream">{document.title}</h2>
          <p className="mt-1 text-xs text-cream/45">{document.filename} · {document.academic_year} · {(document.file_size / 1024).toFixed(0)} KB</p>
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-forest p-6 text-cream">
      <div>
        ...
        <a href="/nac" className="mt-6 inline-block text-sm font-bold text-lime">← Return to archive</a>
      </div>
    </main>
  );
}
